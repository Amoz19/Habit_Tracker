...
const Timer = () => {
  ...
  const [updateHabit, { isSuccess, isError }] = useUpdateHabitMutation();
  ...
  useEffect(() => {
    ...
    if (isActive && time > 0) {
      interval = setInterval(() => {
        ...
        if (FOCUS_TIME - 300 === time && selectedHabitId !== null) {
          updateHabit({
            id: selectedHabitId,
            isComplete,
          })
  ...
  return (
    <div className="w-full h-full bg-gradient-to-b  dark:from-black from-[#e6e6e6] dark:via-[#000000] via-[#ffffff] dark:to-gray-800 to-[#d4e6f1]">
